...
hla_gene_region_file <- NULL
outputdir <- paste0(homedir, "GAMCC/results/hla_test/imputation/batches_test/genes_test/A/")
summary_output_file_prefix <- 'quilt.hla.output'
nCores <- 1
prepared_hla_reference_dir <- paste0(homedir, "GAMCC/results/hla_test/imputation/ref_panel/QUILT_ref_files/")
quilt_hla_haplotype_panelfile <- "" # Is this really an empty string?
final_output_RData_file <- NA
write_summary_text_files <- TRUE
overrideoutput <- FALSE
nGibbsSamples <- 15
